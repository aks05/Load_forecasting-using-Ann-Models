...
df.to_csv("server/predictions/SMA/%s.csv" % date, index=False)
#scp("SMA", date)
...
df.to_csv("server/predictions/SES/%s.csv" % date, index=False)
#scp("SES", date)
...
labels = ["time", "load"]
df = pd.DataFrame.from_records(pred, columns=labels)
df.to_csv("server/predictions/WMA/%s.csv" % date, index=False)
#scp("WMA", date)
